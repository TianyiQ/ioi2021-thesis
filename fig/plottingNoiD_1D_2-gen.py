...
ax2=ax.twinx()
# ax2.plot(nx,nc,'r',label='分数分布',color='green',linestyle='--')
ax.plot(nx,ny_ex,'r',label='$D_1$关于$\\mu$的图像',color='red',linewidth=1.25)
ax2.plot(nx,ny_ex_rel,'r',label='$D_1/\\mathrm{E}[U_p]$关于$\\mu$的图像',linestyle='--',color='red',linewidth=1.25)
ax.plot(nx,ny_med,'r',label='$D_2$关于$\\mu$的图像',color='blue',linewidth=1.25)
ax2.plot(nx,ny_med_rel,'r',label='$D_2/\\mathrm{Med}[U_p]$关于$\\mu$的图像',linestyle='--',color='blue',linewidth=1.25)
ax.set_xlabel('$\\mu$')
ax.set_ylabel('绝对差值')
...
plt.grid(linestyle='--')
plt.savefig(fname="plottingNoiD_1D_2.pdf",format="pdf",bbox_inches='tight',pad_inches=0.05)