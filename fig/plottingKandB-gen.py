...
plt.plot([ally[i] for i in range(leny)],[K[i] for i in range(leny)],'r',color='blue',label='一次项系数关于$\\sigma$的图像')
plt.plot([ally[i] for i in range(leny)],[B[i] for i in range(leny)],'r',color='red',label='常数项关于$\\sigma$的图像')
plt.plot([ally[i] for i in range(leny)],[fun(K[i],B[i]) for i in range(leny)],'r',color='orange',linestyle='--',label='使一次函数值小于自变量的最大自变量取值')
