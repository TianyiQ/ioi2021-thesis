...
\section{选手整体水平的估计}\label{sec:estimationOfOverallDistribution}
    ...
    \subsection{从分数分布还原期望值分布}\label{sec:calculatingXofSecondRound}
        ...
        $$
        \bm{\Delta}_{A^\prime}(\delta)=\frac{1}{\sqrt{2\pi}\sigma}\exp{\left(-\frac{\delta^2}{2\sigma^2}\right)},\quad\forall \delta\in\mathbb{R}
        $$
        进而由命题\ref{prop:scoreDistributionInSimpleContest}得
        \begin{equation}
            \bm{C}_{A^\prime}(s)=\frac{1}{\sqrt{2\pi}\sigma}\int\limits_0^1 \bm{X}_{A^\prime}(x)\exp{\left(-\frac{(s-x)^2}{2\sigma^2}\right)}\mathrm{d}x,\quad\forall s\in\mathbb{R}
        ...
        \begin{theorem}
            对于 $x\in(\epsilon,1]$ 有 $\bm{X}_{A^\prime}(x)=ax^2+bx+c$ ，其中 $a\approx 1.697,b\approx -3.352,c\approx 1.655$ ，$\epsilon$ 为小常数，$A^\prime$ 为复赛在去除初赛影响后对应的理想比赛。
...
\begin{thebibliography}{9}
    ...
    \inserturl{https://docs.scipy.org/doc/scipy/reference/optimize.minimize-bfgs.html\#optimize-minimize-bfgs}
\end{thebibliography}