...
\section{选手整体水平的估计}\label{sec:estimationOfOverallDistribution}
    ...
    虽然由假设\ref{ass:equivalenceBetweenCodingContests}，不同年份的联赛复赛（所对应的理想比赛）是缩放等价的；但它们毕竟不相同，因此“在联赛复赛中的期望分数”这一概念需要澄清。\ref{sec:dataPreprocessingOfNOIP}小节将处理这一问题，并完成对复赛分数数据的初步分析。接着，\ref{sec:calculatingXfromC}小节将从分数数据中，得到复赛（所对应的理想比赛）的期望值分布的表达式。
    ...
    \subsection{计算期望值分布}\label{sec:calculatingXfromC}

        这一小节将计算理想比赛 $A$ 的期望值分布。其中，\ref{sec:dataOfPreliminaryRound}小节将给出初赛分数数据的来源，\ref{sec:assumptionsOnPreliminaryRound}小节将结合这些数据给出关于初赛的几个假设；第\ref{sec:estimationOfOverallDistribution}节的其余部分都将依赖于这些假设。\ref{sec:parameterOfDeltaDistribution}小节将计算理想比赛 $A$ 的偏移量分布的标准差，以为\ref{sec:calculatingXofSecondRound}小节中 $A$ 的期望值分布的计算做好准备。
        ...
        \subsubsection{关于初赛的几个假设}\label{sec:assumptionsOnPreliminaryRound}
            ...
            作出这一假设，意味着忽略不同省市间选手水平上的差异，并用全国整体的选手水平和竞争激烈程度来代替之。
            ...
            \begin{assumption}
                联赛初赛\textbf{在取参赛人群为晋级复赛的全体选手后}所对应的理想比赛，与联赛复赛所对应的理想比赛（即理想比赛 $A$ ）缩放等价。
        ...
        \subsubsection{计算期望值分布}\label{sec:calculatingXofSecondRound}

            在\ref{sec:dataPreprocessingOfNOIP}小节中得到了 $\bm{C}_A(x)$ 的表达式；这一小节将由此计算 $\bm{X}_A(x)$ 的表达式。$\bm{X}_A(x)$的闭合形式很可能并不存在，所以将用级数展开的方式来给出结果；并且由于偏移量分布函数 $\bm{\Delta}_A$ 的参数尚未确定，得到的表达式也将含有参数。
    ...
    \subsection{消除初赛对期望值分布的影响}\label{sec:analysisOnPreliminaryRound}