    ...
    \subsection{对复赛分数的分析}
        ...
        \subsubsection{数据的获取、加工和拟合}\label{sec:dataPreprocessingOfNOIP}
            ...
            \begin{asparaenum}[\bfseries{步骤} 1.]
                ...
                \item 对每场比赛计算分数标准差 $\sigma$ ，然后对分数作变换 $x\mapsto 1-\frac{1-x}{5.52\sigma}$ ，并用变换结果代替原分数。系数$5.52$的作用稍后说明。
                \item 对每场比赛计算最低分，取所得的6个最低分的最大值$T$，并剔除所有$<T$的分数。这一步的理由与步骤\ref{step:noipDataRefinementStep2}中的类似：分数低于$T$的部分选手未能获奖，故将这些选手连同已获奖的那些一并剔除。计算发现 $T\approx 0.2$ ，与步骤\ref{step:noipDataRefinementStep2}中的阈值保持一致；这是系数$5.52$的主要作用。
            ...
            观察这些数据，可以发现数值的分布左侧（即靠近$0.2$一侧）稠密、右侧（即靠近$1$一侧）稀疏，于是尝试用递降的多项式函数、对数函数等常见函数，以及指数分布、截尾正态分布等“向右倾斜”的常见概率分布来拟合之。
