...
\section{选手整体水平的估计}\label{sec:estimationOfOverallDistribution}
    ...
    \subsection{对复赛分数数据的获取、加工和拟合}\label{sec:dataPreprocessingOfNOIP}
    ...
    \subsection{计算期望值分布}\label{sec:calculatingXfromC}
        ...
        这一小节将计算理想比赛 $A$ 的期望值分布。其中，\ref{sec:assumptionsOnPreliminaryRound}小节将给出关于初赛的几个假设；第\ref{sec:estimationOfOverallDistribution}节的其余部分都将依赖于这些假设。\ref{sec:parameterOfDeltaDistribution}小节将计算理想比赛 $A$ 的偏移量分布的标准差，以为\ref{sec:calculatingXofSecondRound}小节中 $A$ 的期望值分布的计算做好准备。

        \subsubsection{有关初赛的几个假设}\label{sec:assumptionsOnPreliminaryRound}

        \subsubsection{计算偏移量分布的参数}\label{sec:parameterOfDeltaDistribution}

        \subsubsection{计算复赛的期望值分布}\label{sec:calculatingXofSecondRound}

            上一小节中得到了 $\bm{C}_A(x)$ 的表达式；这一小节将由此计算 $\bm{X}_A(x)$ 的表达式。$\bm{X}_A(x)$的闭合形式很可能并不存在，所以将用级数展开的方式来给出结果；并且由于偏移量分布函数 $\bm{\Delta}_A$ 的参数尚未确定，得到的表达式也将含有参数。
