...
\section{关于比赛名次的讨论}
    ...
    \subsection{比赛名次与实际水平的关系}
        ...
        \subsubsection{期望分数与比赛名次的关系}
            ...
            记随机变量 $C_p$ 为 $p$ 的（与 $B$ 中的选手分数一起，按定义\ref{def:realIdealCorrespondence}中的方式变换过后的）分数，设其服从期望值为 $\mu$ 、标准差为 $\sigma$ 的正态分布，并用 $\mathcal{C}_p$ 来表示其概率密度函数。
            ...
            \begin{align*}
                ...
                &=\int\limits_{-\infty}^{+\infty} \mathcal{C}_p(t)\left(\int\limits_t^{+\infty} \mathcal{C}_A(s)\mathrm{d}s\right)\mathrm{d}t \\
                &=\int\limits_{-\infty}^{+\infty} P_{\sigma^2}(t-\mu)\left(\int\limits_t^{+\infty} \mathcal{C}_A(s)\mathrm{d}s\right)\mathrm{d}t \\
                &=\int\limits_{-\infty}^{+\infty} R_{\sigma^2}(s-\mu)\mathcal{C}_A(s)\mathrm{d}s
            \end{align*}

            而对于后者有
            $$
            \mathrm{Med}\left[U_p\right]=\int\limits_{\mu}^{+\infty} \mathcal{C}_A(s)\mathrm{d}s
            $$
            ...
            由此可以对给定的 $\mu,\sigma,\mathcal{C}_A$ 计算 $D_1,D_2$ 的值。
