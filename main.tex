...
\section{选手整体水平的估计}\label{sec:estimationOfOverallDistribution}
    ...
    \subsection{结合初赛的分析}\label{sec:analysisOfBothRounds}
        ...
        \subsubsection{计算偏移量分布的参数}\label{sec:parameterOfDeltaDistribution}
            ...
            \begin{asparaitem}
                \item 计算发现第35大的差值约等于$0.28$，高于初赛晋级分数线经过变换后的值。又因为复赛分数不可能小于$0$，所以任何一个未达到晋级分数线的选手，其两试分数差值不可能达到$0.28$。（计算发现35人中最低的初赛分数为63分，比晋级分数线高出约20分）
            ...
            \begin{asparaenum}[\bfseries{步骤} 1.]
                ...
                \item 计算北京初赛前417名的分数标准差 $\sigma_1$ ，再对映射后的复赛分数计算北京选手（共417名）的分数标准差 $\sigma_2$ 。然后对于初赛分数 $s\in [0,100]$ ，将其映射到 $1-(1-\frac {s}{100})\cdot\sigma_2/\frac{\sigma_1}{100}$ 。由命题\ref{prop:alphaAsQuotientOfStddev}，这一映射方式保证了映射后两个理想比赛相同。
            \end{asparaenum}
            ...
            这里采用最大似然估计，即选取一个 $\sigma$ 以最大化：在全体差值服从 $N(0,2\sigma^2)$ 的条件下，测量得到 $d_1,\cdots,d_{35}$ 的概率。注意到这个概率实际上必定等于0，但可以通过取极限规避这一问题。下式给出最优的 $\sigma$ 的计算方式，其中 $u_{1},\cdots,u_{781}$ 表示随意排列的 $781$ 个差值，$v_{k}$ 表示 $u_{1},\cdots,u_{781}$ 中的第 $k$ 大值。
            ...
            \begin{align*}
                ...
                &=\argmax\limits_{\sigma\in\mathbb{R}_{>0}} \left(\prod\limits_{k=1}^{35} R^\prime_{2\sigma^2}(d_k)\right)\cdot R_{2\sigma^2}(d_{35})^{781-35} \\
            ...
            至此，问题完全转化为一个最优化问题。最优化方法采用SciPy提供的BFGS算法的实现\cite{scipy_minimize}，算得$\sigma\approx 0.12224$。
            ...
            \begin{proposition}
                $$
                \bm{\Delta}_{T\left[F_1\right]}(t)=\bm{\Delta}_{F_2}(t)=P_{\sigma_F^2}(t)
                $$
                
                其中“缩放变换” $T$ 满足 $T\left[F_1\right]=F_2$，常数 $\sigma_F$ 约等于 $0.12224$ ，$t$ 为任意实数。
...
\begin{thebibliography}{9}
    ...
    {\newline\scriptsize\url{https://en.wikipedia.org/wiki/Sum\_of\_normally\_distributed\_random\_variables}}

    \bibitem{scipy_minimize} 
    SciPy Documentation: scipy.optimize.minimize,
    {\newline\scriptsize\url{https://docs.scipy.org/doc/scipy/reference/optimize.minimize-bfgs.html\#optimize-minimize-bfgs}}
\end{thebibliography}